...
**Overall:** **DEPLOYED & LIVE 2026-07-03.** All 7 phases shipped to `dev` (each ≥3-round gate clean); **PR open: [#1](https://github.com/scarson/artifact-share/pull/1)** (`dev` → `main`, unmerged). A follow-up replaced admin password+TOTP with **Cloudflare Access + Google SSO** (owner in Google Advanced Protection — see the top Deviations + spec §8/§15 Q6). The owner ran the account hand-off: both D1 DBs created (prod `220fd2d6`, preview `37eeeefc`), migrations 0001+0002 applied remote, `meta` markers set, `ASSET_COOKIE_SECRET` set, both envs **deployed manually** via `wrangler deploy`. Live-verified: `share.scarson.io/admin` → 302 to Cloudflare Access; `/a/*` fail page; `/robots.txt`; blank root; preview inert. **Session handoff: [`docs/HANDOFF.md`](../HANDOFF.md).** Remaining (owner/next agent): end-to-end mint→redeem test (needs an admin Google login), CI repo secrets + confirm Workers Builds disabled before merging `main`, then merge PR #1.
...
| 7 — Environments, deploy pipeline & isolation | ✅ Deployed & live 2026-07-03 | fe63660, 24f85c7, 4eb8f78, fa1cf84 | owner ran the hand-off: prod DB 220fd2d6 + preview DB 37eeeefc created, migrations 0001+0002 applied remote, meta markers set, ASSET_COOKIE_SECRET set, both envs deployed. Live-verified: `share.scarson.io/admin` → 302 to Cloudflare Access; `/a/*` failure page renders; `/robots.txt` ok; preview inert. Remaining: end-to-end mint→redeem test (needs an admin Google login) + CI repo secrets + confirm Workers Builds disabled. |

### Deviations
- **Post-deploy fixes + UI round (2026-07-03, owner-requested):** (a) **Live bug fixed — admin form POSTs 403'd**: the global `Referrer-Policy: no-referrer` made browsers serialize the Origin header of the panel's own same-origin form POSTs as the literal `"null"` (Fetch spec) and omit Referer, so the pinned-origin CSRF check rejected every Generate/Revoke. Authorized panel responses now carry `Referrer-Policy: same-origin` (denied/failure responses keep `no-referrer`; header parity with gate failures tested). Reproduced + verified in a real browser against `wrangler dev`. (b) **Root page**: spec §9's blank `/` replaced (owner-requested) with a minimal neutral identity page — host wordmark + "invitation only", still nothing to enumerate (no links, no admin mention; pinned by smoke test). (c) **UI polish (`/admin`, failure page, root)**: shared visual system in `src/lib/ui/styles.ts`; inline `<style>` blocks allowlisted by **sha256 hash** in ADMIN_CSP (still no `unsafe-inline`; `csp.test.ts` recomputes the hashes), `img-src data:` added for an inline SVG favicon; failure page remains ONE constant (byte parity preserved). (d) **Test hermeticity**: vitest bindings pin `ACCESS_DEV_BYPASS: "0"` — the pool loads `.dev.vars`, whose documented local-QA bypass otherwise disabled Access enforcement in deny tests. (e) **CI**: deploy job preflights the repo secrets with an actionable error (root cause of all red deploy runs = secrets never added; owner action, SETUP.md §5).
- **Post-plan admin-auth change (2026-07-03, owner-ratified — Access replaces password+TOTP):** as a follow-up on top of PR #1, the admin login was switched from the plan's argon2id-password + TOTP (Phase 4) to **Cloudflare Access + Google SSO** (spec §8 banner / §15 Q6 flipped from "layer" to "replace"; owner is in Google Advanced Protection, making an app-maintained second factor redundant). New: `src/lib/auth/cfaccess.ts` verifies the `Cf-Access-Jwt-Assertion` JWT (jose remote-JWKS, RS256/issuer/audience pinned, admin-email allowlist, fail-closed); `src/routes/admin.ts` guard now = production env-gate → Access verification, with an `ACCESS_DEV_BYPASS=1` local-dev bypass confined to `.dev.vars` and **linted out of committed config** (`hasDevBypass()` in the build). Removed: `password.ts`/`totp.ts`/`session.ts`/`totpStore.ts` + tests, `signSession`/`verifySession`, `loginThrottleMs`, the `ADMIN_PASSWORD_HASH`/`ADMIN_TOTP_SECRET`/`SESSION_SECRET` secrets, and the `@noble/hashes`/`otpauth` deps. Kept: production-only gate, pinned-origin CSRF on mutations, the whole `/a/*` recipient gate. Commits 75c1647, b3a1b26, a941a5a (+ docs). This supersedes Phase 4's password/TOTP tasks in the running system; the Phase 4 plan text is retained as historical record. `secrets.required` is now just `["ASSET_COOKIE_SECRET"]`; the ACCESS_* config is non-secret vars in wrangler.jsonc.
