...
**Project override (owner grant, 2026-07-03).** Sam granted agents merge authority for ALL PRs — including `main` publication PRs — PROVIDED a multi-round adversarial review runs first: alternating rounds with blind subagents (fresh context, findings-only), continuing until a round converges at zero substantive findings. The classification discipline below still applies (label every PR); the "user merges Review-class" default is superseded by review-then-merge, EXCEPT when the adversarial review itself leaves genuine doubt on a security-sensitive change — then stop and leave the PR for Sam with the concern written in the PR body.
...
**Deploy coupling (this project):** `.github/workflows/deploy.yml` deploys the PREVIEW Worker on every push to `dev` and PRODUCTION on every merge to `main`. Publishing = deploying. A single working session MAY fast-forward-push its reviewed feature branch directly onto `dev` (the practiced convention here — preview CI validates it); `main` only ever advances via the publication PR below.
