...
artifact-share — a single-admin gated sharing site for confidential self-contained HTML assets, running as one Cloudflare Worker (Hono) with D1 (codes/metadata) and R2 (asset bytes). Recipients redeem per-recipient access codes at `/a/<slug>?code=…`; every request re-checks the code in D1 and fails closed (instant revocation). Admin panel at `/admin` sits behind Cloudflare Access + Google SSO.

Authoritative docs, in read order for new work:
- Spec: `docs/design/2026-07-02-gated-asset-sharing-site-design.cloudflare.md` (§-references throughout the code point here)
- Asset-manager + recoverable-codes design: `docs/design/2026-07-03-asset-manager-r2-and-recoverable-codes-design.md`
- Active plan: `docs/plans/2026-07-03-asset-manager-r2-and-recoverable-codes-plan.md` (Living Document Contract — keep its Execution Status current)
- Pitfalls (read before coding): `docs/pitfalls/implementation-pitfalls.md`, `docs/pitfalls/testing-pitfalls.md`
...
- **Scope.** "Feature or bugfix" means production code under `src/`. Migrations (`migrations/*.sql`) count — they get schema-shape tests in `src/lib/db/schema.test.ts`. TDD does NOT apply to: documentation (`docs/`, `*.md`), configuration (`wrangler.jsonc`, `*.json`, `*.yml`), `scripts/` (covered by targeted unit tests where load-bearing, e.g. the config lints), CI (`.github/`), or the generated modules under `.generated/` (never hand-edit those; regenerate via `npm run build-manifest` while that pipeline still exists).
...
- Defense in depth isn't a DRY violation. Layered validation (interactive → command → server) or redundant checks on high-stakes operations are features, not smells — DRY governs code quality, defense in depth governs security and correctness. When they conflict, defense in depth wins. (This repo leans hard on it: gate re-checks, config lints, hashed CSP + escaping, CSRF on top of Access.)
...
## Code Comments
 ...
 - **This repo's file-header convention** (instead of the generic ABOUTME pattern): every source file opens with a `/** … */` doc comment stating what the module does and which spec section (§N) makes it load-bearing. Match it. Inline comments carry the WHY — especially security invariants ("fail closed", "single time source", "never branch the response") — and cite the spec §. Preserve these citations when editing.

  Example (GOOD, from `src/lib/http/csrf.ts`): `/** True iff this state-changing request is same-origin with PUBLIC_ORIGIN (spec §8 — pinned origin, NOT the request's own Host). */`
  Examples (BAD): `// new improved CSRF check`, `// moved from admin.ts`, `// TODO: cleanup later`
...
- **Allowed types:** `feat`, `fix`, `chore`, `docs`, `refactor`, `test`, `perf`, `build`, `ci`. Common scopes in this repo: `admin`, `gate`, `vault`, `assets`, `ui`, `config`, `deploy`, `plan`.
...
- **Branch roles:** `main` is the live/deploy branch (every merge to `main` deploys production via `.github/workflows/deploy.yml`); `dev` is the permanent integration branch (pushes to `dev` deploy the preview Worker). Feature work branches off `dev`; flow is feature branch → `dev` → PR → `main`. NEVER push `main` directly.
...
- **Worktrees live at `.claude/worktrees/<slug>` inside the repo, NOT as siblings of the repo directory.** The path is gitignored. `git worktree add .claude/worktrees/<slug> -b <branch-name>` creates both in one step.
...
- **Merge authority (owner grant, 2026-07-03):** agents MAY merge PRs to `main` themselves PROVIDED a multi-round adversarial review ran first — alternating rounds with blind subagents (fresh context, findings-only), continuing until a round converges at zero substantive findings. This applies to every PR regardless of size. Security-sensitive uncertainty (auth, secrets, crypto, gate semantics, data integrity) where the review itself leaves doubt → STOP and leave the PR for Sam with the concern written in the PR body. When CI fails, the agent investigates and fixes (up to 3 attempts on the same failure before escalating). Conflicts: rebase in the worktree (not GitHub UI), `git push --force-with-lease` (never plain `--force`). Always `gh pr merge --merge` — never `--squash`, never `--rebase`.
...
- This repo's specifics: tests run INSIDE workerd via `@cloudflare/vitest-pool-workers` (`cloudflare:test`, real D1/R2 — no Node shims, no mocks of the platform); bindings are pinned in `vitest.config.ts`, never in env files; per-test DB reset comes from `src/test/apply-migrations.ts` — do NOT use `test.concurrent` within a file. Full list: `docs/pitfalls/testing-pitfalls.md`.
...
2. User-scoped memory (Claude's project memory at `~/.claude/projects/<slug>/memory/`, or your agent framework's equivalent user-scoped store). Prevents regressions at session-restore time because future sessions auto-load recent learnings.
...
```bash
npm test                                  # full suite (vitest, runs inside workerd)
npx tsc --noEmit                          # typecheck
npm run build-manifest                    # regen .generated/ + config lints (until the R2 cutover retires it)
npx wrangler dev                          # local dev on :8787 (needs .dev.vars — cp .dev.vars.example .dev.vars)
npx wrangler deploy --dry-run --env production   # validate config without deploying
npx wrangler d1 migrations apply artifact-share-dev --local   # local schema
```

Deploys are CI-driven: push `dev` → preview Worker; merge `main` → production. Manual deploy is the emergency path only (`npx wrangler deploy --env production`).

## Tech Stack

| Layer | Choice |
|---|---|
| Runtime | Cloudflare Workers (single Worker, Hono router) |
| Data | D1 (SQLite) — codes, rate limits, asset metadata; single time source (`unixepoch()`) |
| Content | R2 private bucket via binding (per the 2026-07-03 asset-manager design; bundled Text modules until that ships) |
| Admin auth | Cloudflare Access + Google SSO; Worker independently verifies the JWT (jose, remote JWKS) |
| Crypto | WebCrypto only (SHA-256, HMAC, AES-GCM); no native/wasm KDFs — they don't run on Workers |
| Tests | vitest 4 + `@cloudflare/vitest-pool-workers` (real D1/R2 in workerd) |
| CI/CD | GitHub Actions (`.github/workflows/deploy.yml`): test job gates deploy job |

## Architecture (Key Points)

- One Worker, three surfaces: public gate (`/a/:slug`), admin panel (`/admin*`, Access-gated), and neutral public pages (`/`, failure page, `robots.txt`).
- **Everything fails closed** to ONE byte-identical generic failure page (`src/lib/failure.ts`) — unknown slug, bad code, revoked, expired, DB error, wrong environment, unknown route. No response may fingerprint which failure occurred.
- Redemption is a single atomic D1 `UPDATE…RETURNING`; recipients then hold a signed HttpOnly cookie, but EVERY later load re-checks the code row in D1 (instant revocation). Cookie lifetime is capped by code expiry.
- Codes are stored as SHA-256 for lookup (plus, post-vault, AES-GCM ciphertext for admin recovery — never plaintext, never logged).
- `ENVIRONMENT` var is the only environment oracle: only `production` serves traffic; preview/dev render the generic page. Preview is a separate Worker + empty D1 behind Access.
- Security headers + CSP are applied by ONE finalizing middleware in `src/index.ts`; inline styles/scripts are sha256-allowlisted (`src/lib/http/csp.test.ts` guards the hashes). No `unsafe-inline`, ever.
- The wrangler.jsonc `assets` key is BANNED (no platform static surface); the build lints it.

## Conventions

- Spec §-references in comments are load-bearing — they anchor code to `docs/design/2026-07-02-…cloudflare.md`. Keep them accurate when moving code.
- Slugs and codes are 22-char base64url 128-bit tokens (`src/lib/codes.ts`); the server mints both — clients never choose identifiers.
- Admin mutations are POST forms with `originOk` (pinned `PUBLIC_ORIGIN`) CSRF checks; admin panel pages carry `Referrer-Policy: same-origin` (NOT `no-referrer` — that serializes the Origin header as `"null"` on same-origin POSTs and breaks the check; see the pitfalls doc).
- UI styles live in `src/lib/ui/styles.ts` as string constants; editing them requires re-hashing in `src/lib/http/headers.ts` (the csp test prints the correct hash on mismatch).
- Plans and design docs follow the Living Document Contract (per-phase Execution Status banners, Deviations/Discoveries at top) — update them at ship time, not retrospectively.

## Language / Framework Gotchas

READ `docs/pitfalls/implementation-pitfalls.md` for the full list. Critical items:

- **Fail closed on DB/R2 error** — catch, render the generic page, never a 500 with details.
- **D1 is the single time source** — `unixepoch()` in SQL; never `Date.now()` for authorization decisions.
- **SQLite `ALTER TABLE ADD COLUMN` cannot take expression DEFAULTs** — plain nullable columns in follow-up migrations.
- **`secrets.required` in wrangler.jsonc is per-env, not inherited** — it lives inside each `env` block.
- **`wrangler d1 create` / bucket-create auto-add prompts: DECLINE** — they append wrong-named top-level bindings; paste IDs into the env blocks by hand.
- **`ACCESS_DEV_BYPASS` lives ONLY in `.dev.vars`** — the build fails if it appears in committed config; tests pin it to `"0"` in `vitest.config.ts`.
- **hash-wasm / native KDFs don't run on Workers** (runtime `WebAssembly.compile` is forbidden) — WebCrypto only.

### Universal Gotchas

- **No secrets in CLI flags or command-line env var overrides.** Credentials come from files, keychain, prompts, or scoped environment — never `--secret` / `--password` flags. Visible in `ps` and shell history. (Piping into `wrangler secret put` is the house pattern.)
- **No PII in audit/debug logs.** Log identifiers (entry IDs, correlation IDs, command names) — never field values, raw codes, or document content.
...
- Execute plans task-by-task with TDD; per-phase group reviews run ≥3 adversarial rounds (alternate self-review with blind subagents) and continue until a round is clean.
- Update the active plan's Execution Status banners at claim/ship/defer time (Living Document Contract).
- After each phase ships: update `docs/pitfalls/*.md` with anything learned, then PR `dev → main`.
...
artifact-share/
  src/
    index.ts                # app assembly: finalizing header middleware, root page, route mounting
    routes/                 # gate.ts (/a/:slug), admin.ts (/admin*)
    lib/                    # codes, failure page, envgate, ratelimit, manifest
      auth/                 # cfaccess.ts — Access JWT verification
      crypto/               # signed asset-cookie tokens (key ring)
      db/                   # adminRepo, rateStore, schema tests
      http/                 # headers (CSP + hashes), csrf
      ui/                   # styles.ts — hashed inline CSS/JS for first-party pages
    test/                   # apply-migrations reset, smoke, seeds
  migrations/               # forward-only D1 migrations (never edit shipped ones)
  assets/ + .generated/     # bundled-content pipeline (retired by the R2 asset-manager plan)
  scripts/                  # build-manifest + config lints
  docs/                     # design/ (specs), plans/ (living plans), pitfalls/, deploy/SETUP.md
  .github/workflows/        # deploy.yml — test job gates deploy; dev→preview, main→production
```
...
| `superpowers-plus:writing-plans-enhanced` | Before multi-step implementation (wraps `superpowers:writing-plans` with this repo's conventions) |
| `superpowers-plus:plan-review-cycle` | After writing a plan, before committing it |
...
| `superpowers-plus:handoff` | When context is about to be lost (long session, pre-compaction) |
| `impeccable` (`/impeccable`) | Any UI design/polish work on the first-party pages |

**When to dispatch parallel subagents on this project:** adversarial review rounds (blind reviewers on plans/PRs — the house merge-authority requirement), bug hunts across the security invariants, and independent plan phases (e.g. vault work vs. UI work). Keep gate-semantics changes single-threaded — they're too coupled for parallel edits.

## Skill routing

When the user's request matches an available skill, you MUST invoke it using the Skill tool as your FIRST action. Do NOT answer directly, do NOT use other tools first.

Key routing rules:
- Bugs, errors, "why is this broken" → `superpowers:systematic-debugging`
- New feature / "let's add X" → `superpowers:brainstorming` first
- Writing implementation plans → `superpowers-plus:writing-plans-enhanced`
- Review a plan before committing → `superpowers-plus:plan-review-cycle`
- UI/design work → `impeccable`
- Prose for humans (README, announcements) → `stop-slop`
